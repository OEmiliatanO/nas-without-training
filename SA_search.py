...
Rt = 0.6
T = 1
def neighbor(index, uid):
    # just pick next
    if index == None:
        aiter = np.random.randint(0, len(searchspace))
        return aiter, searchspace[aiter]
    return (index+np.random.randint(0,len(searchspace))%5)%len(searchspace), searchspace[(index+np.random.randint(0,len(searchspace))%5)%len(searchspace)]
##
...
for N in runs:
    ...
    # SA
    #for arch in indices:
    best_sol_uid = now_sol_uid = None
    best_sol_index = now_sol_index = None
    best_sol_E = now_sol_E = 1e-10
    T = 1
    while T > EndT:
        for i in range(maxIter):
            #uid = searchspace[arch]
            nex_sol_index, nex_sol_uid = neighbor(now_sol_index, now_sol_uid)
            #if nex_sol_uid == None:
            #    print("nex_sol_uid is none")
            #print(nex_sol_uid)
            network = searchspace.get_network(nex_sol_uid)
            network.to(device)
            if args.dropout:
            ...
            #print(now_sol_E)
            dt = s - now_sol_E
            if dt > 0 or np.log(np.random.randint(0,100)/100) < -dt/T:
                now_sol_uid = nex_sol_uid
                now_sol_E = s
                now_sol_index = nex_sol_index
            if s > best_sol_E:
                best_sol_uid = nex_sol_uid
                best_sol_E = s
                best_sol_index = nex_sol_index
            
            #print("best_sol_E = ", s, " ,best_sol_uid = ", best_sol_uid)
            #scores.append(s)
        T *= Rt
    #print(len(scores))
    #print(scores)
    ...
    if best_sol_uid == None:
        print("best_sol_uid is None")
    uid = best_sol_uid
    #topscores.append(scores[order_fn(scores)])
    topscores.append(best_sol_E)
    #chosen.append(best_arch)
    chosen.append(best_sol_index)
